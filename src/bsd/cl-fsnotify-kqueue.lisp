...
(defun add-watch (kq path)
  (unless (null (probe-file path))
    (let ((fd (or
               (gethash path (kqueue-hash kq))
               (with-foreign-string (c-path path)
                 (foreign-funcall "open" :string c-path :int (logior open-read-only open-non-block) :int)))))
...
(defun get-event (kq-fd &key (blockp nil))
  (with-foreign-objects ((kev 'struct-kevent) (timespec 'struct-timespec))
    ...
    (when (= (c-kevent kq-fd (null-pointer) 0 kev 1 (if blockp (null-pointer) timespec))
             1)
      (convert-from-foreign (foreign-slot-value kev 'struct-kevent 'udata) :string))))

(defun get-events (kq &key (blockp nil))
  (loop as event = (get-event (kqueue-fd kq) :blockp blockp) then (get-event (kqueue-fd kq))
        while event
        collect event))
